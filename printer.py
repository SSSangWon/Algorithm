...
내가 인쇄를 요청한 문서가 몇 번째로 인쇄되는지 return 하도록 
solution 함수를 작성해주세요.
...
def solution(priorities, location):
    ...
    while priorities:
        answer+=1
        maxNum=priorities.index(max(priorities))

        if location > maxNum:
            location=location-maxNum-1
        
        elif location < maxNum:
            location = len(priorities)-maxNum+location-1
        
        elif location == maxNum:
            return answer
        priorities=priorities[maxNum+1:]+priorities[:maxNum]
...
l=0
print(solution(p,l))
#print(max(p),p[max(p):]+p[0:len(p)-1])